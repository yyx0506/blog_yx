...
顾名思义 coker compose 是指容器编排

定位是定义和运行多个docker 容器的应用,同时可以对多个容器进行编排（可以指定顺序和依赖）
...
### 1:compose 核心概念 服务（serves)

一个应用的容器，实际上可以包括若干运行相同镜像的容器实例，服务可以存在多个

**项目（project)**  **: 由一组关联应用容器组成的一个完整的业务单元，在docker-compose.yml中定义**

### 2:docker-compose的使用(只针对linux)

#### 1:可以选择版本 下载docker-compose
...
```
vim docker-compose.yml 可以起不同的名字 默认是此， 起其他名字执行时需要 使用-f 指定文件
```

```
# 编译 docker-compose.yml 文件
# 指定项目版本 一般是2.2-4.0 第二步写ser
version: '3.1'
services:
    monitor_ad_list: # 服务名随意写 必须唯一
        image: nginx # 使用的镜像 如果不带tag 默认最新版本
        container_name: monitor_ad # 容器名字
        ports: 
            - 8080:8080 # 如果有需要的话 后面的是容器内的端口 前面的是容器外的端口
        volumes:   #. 
            - 容器外目录:容器内目录  # 挂载
            - /etc/localtime:/etc/localtime # 将容器外时间和容器内时间统一
        restart:  # 重启
            always
        logging:  # 日志信息
            driver: "json-file"
...
```
docker-compose up -d # 上线yml文件中配置的容器 -f 可以指定文件，默认直接使用docker-compose.yml 
docker-compose down -v 停止并删除镜像
...
docker-compose scale test=80 # 开启80个test容器 对应的
docker-compose -f compose_console_word.yml scale  任务名称=数量
```

#### 小 tips: docker-compose 如果出现容器间通信失败有可能是内网冲突。重启服务器即可
