...
Go 并不是一种静止的，一成不变的编程语言，新的功能是经过大量的讨论和实验后慢慢采用。
例如 1.1的modules,1.7context,1.18后包括了类型参数的视线，也就是俗称的泛型，泛型的场景没有那么常见，但是
在某些场景下还有必要的
下面我们看一个例子
...
看起来是没问题的但是运行后就报错cannot use str (variable of type []string) as []interface{} value in argument to printArray

如果按照传统的方法的话就需要去通过类型断言去处理
...
// 定义一个万能接口
func printArray(arr interface{}) {
	// 类型断言 x.(T) 其实就是判断 T是否实现了 x 接口，如果视线了，就把X接口类型具体化为 T类型
...
```
 ...
 如果我有其他的不同类型需要断言的话就太过于麻烦了，所以说需要一个方法或者模式去处理其他的问题 

```

// 我们不应该限定他的类型，让调用者自己去定义类型 在有的语言里面使用的<> go 使用的是 []
func printTArray[T string | int](arr []T) int {
	for _, a := range arr {
		fmt.Println("a:", a)
	}
	return 1
...
func main() {
	str := []string{"abc", "dddd"}
	ints := []int{1, 2}
	result1 := printTArray(ints)
	result2 := printTArray(str)
	fmt.Println(result1, result2)
}
```

这个就是形式类型，GO也给我们内置了泛型类型 any 和 comparable 

any 表示go里面所有内置的可比较类型 comparable表示：'int','uint','float','bool','struct','指针'等一切可以比教的类型

泛型的作用：泛型减少了重复代码并提高了类型安全性

#### 2：泛型的详解

具体的大家可以看一下官方文档的介绍 https://go.dev/ref/spec

引用文档 https://segmentfault.com/a/1190000041634906
...
> 💡 Go语言项目中就曾经有人提出过把Go语言中所有 interface{ }替换成 any 的 [issue](https://link.segmentfault.com/?enc=wvy6Yv%2Fc5EfOq03o33At2g%3D%3D.xD01JczqIWqky2evwG2qv0rGJg3AUriywNJkWJmPuqVXFwkMWwOlaC4xLDtj4Swa)，然后因为影响范围过大过而且影响因素不确定，理所当然被驳回了
...
> 💡 这里虽然可以直接使用官方包 [golang.org/x/exp/constraints](https://link.segmentfault.com/?enc=gWIuzKRQ0DCVsz0VrHVv9w%3D%3D.FJhVmtYrVA4j23kL6Aa%2F8nTG4DDEJxcxw2vWm1EahjeIlZh4TMk9FKi1ihP%2B7cVs) ，但因为这个包属于实验性质的 x 包，今后可能会发生非常大变动，所以并不推荐直接使用
